...
export async function composeDailyPayload(
  ...
  const prompt = {
    ...
    required_output_shape: {
      ...
      scene_prompt: '45 to 90 words of source-led image-generation art direction for gpt-image-2: name the visual world, composition archetype, camera/plate grammar, strongest visible moves, light, palette, and source-bearing anchor treatment; no markdown, no rationale, no product/implementation explanation',
      artifacts: [
        {
...
export async function generateScenePlate(
  ...
  await fs.writeFile(path.join(runDir, 'scene-prompt-full.json'), `${JSON.stringify({
...
function normalizeSourceImageFingerprints(value) {
  if (!Array.isArray(value)) return []
  return value
    .filter((fingerprint) => fingerprint?.image_url)
    .slice(0, 5)
...
function compactText(value, maxLength = 260) {
  ...
  return `${clipped.slice(0, wordEnd > 80 ? wordEnd : maxLength).trim()}…`
}

function joinLimited(values, fallback, limit = 4) {
  const result = normalizeStringArray(values, []).slice(0, limit)
...
export function buildSceneImagePrompt(payload) {
  ...
  const graphicEditorialGuard = looksLikeGraphicEditorialSource(payload, preserveText)
    ? 'This is a graphic/editorial/poster/package reference: borrow layout grammar, figure/portrait masses as abstract shapes, text-strip silhouettes as illegible marks, object/hand edge cues, negative space, palette, and light. Do not preserve the exact cover/card/photo arrangement. Recompose those masses into a new plate while staying flat unless the source itself has deep space. Do not replace it with unrelated macro texture, cave, room, landscape, city, skyline, horizon, character, central figure, poster wall, or metaphor.'
    : ''
