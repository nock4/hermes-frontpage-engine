...
function isArtworkAnchor(source) {
  const text = sourceText(source).toLowerCase()
  const url = String(source?.url || source?.source_url || source?.final_url || '').toLowerCase()
  const artworkLanguage = /\b(artist|artwork|drawings?|painting|paintings|gallery|museum|ceramics?|sculpture|collage|illustration|photography|photo|posters?|prints?|zine|comic|textile|fashion|installation|exhibition|graphic design|visual culture|archivepilled|compositioning|hollycurates|monet|jans muskee|ken white)\b|\bhir[oō]o?\s+isono\b/.test(text)
